...
func init_zombie_wave_create_manager(game_para:ResourceLevelData):
	...
	## 丢车(小推车被消耗)后, 该行一段时间内不再出怪(原版行为)
	EventBus.subscribe("lawn_mover_used",
		func(payload = []):
			var lane: int = int(payload[0]) if payload.size() > 0 else -1
			zombie_choose_row_system.on_mower_lost(lane))
