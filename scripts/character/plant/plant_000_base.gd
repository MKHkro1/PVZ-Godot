...
## 种植落地小弹跳(挤压伸展),基于body原始缩放做相对变化
func _plant_pop():
	var origin_scale := body.scale
	var tween:Tween = create_tween()
	body.scale = origin_scale * Vector2(0.75, 1.2)
	tween.tween_property(body, "scale", origin_scale * Vector2(1.12, 0.9), 0.1)\
		.set_trans(Tween.TRANS_QUAD).set_ease(Tween.EASE_OUT)
	tween.tween_property(body, "scale", origin_scale, 0.16)\
