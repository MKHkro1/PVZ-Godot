...
## 阳光数字跳动动画(数值变化时的反馈感)
var _tween_sun_punch: Tween

func _punch_sun_counter() -> void:
	if not is_instance_valid(curr_sun_value) or not curr_sun_value.is_inside_tree():
		return
	if _tween_sun_punch and _tween_sun_punch.is_valid():
		_tween_sun_punch.kill()
	curr_sun_value.pivot_offset = curr_sun_value.size / 2.0
	curr_sun_value.scale = Vector2(1.28, 1.28)
	_tween_sun_punch = create_tween()
	_tween_sun_punch.tween_property(curr_sun_value, "scale", Vector2.ONE, 0.18)\
