...
## 冒险模式植物解锁(原版: 每通关一关解锁一个植物, 初始仅豌豆射手)
var curr_unlocked_plants:Array[int] = [PlantType.P001PeaShooterSingle]
## 每关奖励植物表[大关-1][关内序号-1] (与选关界面预览一致; 道具/纸条关沿用相邻植物占位)
const AdventureRewardPlants:Array = [
	[2, 3, 4, 4, 5, 6, 7, 8, 8, 9],
	[10, 11, 12, 13, 13, 14, 15, 16, 16, 16],
	[17, 18, 19, 20, 22, 21, 23, 24, 24, 20],
	[25, 26, 27, 28, 29, 30, 31, 32, 32, 26],
	[33, 34, 35, 36, 37, 38, 40, 40, 40, 33],
]

## 通关冒险关卡(level_index = level_id-1)解锁对应奖励植物; 返回是否为新解锁
func unlock_adventure_reward_plant(level_index:int, silent := false) -> bool:
	if level_index < 0 or level_index >= AdventureRewardPlants.size() * 10:
		return false
	var stage:int = floori(level_index / 10.0)
	var pos:int = level_index % 10
	var plant_type:int = AdventureRewardPlants[stage][pos]
	if plant_type in curr_unlocked_plants:
		return false
	curr_unlocked_plants.append(plant_type)
	if not silent:
		print("解锁新植物: P%03d" % plant_type)
	return true
...
func load_global_game_data() -> void:
	...
	## 植物解锁存档(旧档无此字段时按通关进度回填)
	var saved_unlocked:Variant = data.get("curr_unlocked_plants", [])
	if saved_unlocked is Array and not (saved_unlocked as Array).is_empty():
		curr_unlocked_plants.clear()
		for p:Variant in saved_unlocked:
