...
#region 播放音乐和音效
## 场景 BGM 预载注册表: 供场景切换时黑屏期间即刻起播, 消除"进场景后音乐晚一两秒"
const BGM_TRACKS := {
	&"start_menu": preload("res://assets/audio/BGM/start_menu_bgm.mp3"),
	&"choose_card": preload("res://assets/audio/BGM/choose_card.mp3"),
	&"garden": preload("res://assets/audio/BGM/garden.mp3"),
}

func play_bgm(stream: AudioStream):
	## 同曲目已在播: 保持连续不从头重播(跨场景衔接无缝)
	if bgm_play.stream == stream and bgm_play.playing:
		return
	## 运行时强制循环(防止导入参数漂移导致曲末静音)
	if stream is AudioStreamMP3:
		stream.loop = true
	bgm_play.stream = stream
	bgm_play.play()

## 按注册表 key 播放 BGM(配合 SceneTransition 场景入场映射)
func play_bgm_key(key: StringName) -> void:
	play_bgm(BGM_TRACKS[key])

## 平滑换曲: 压低音量→切流→恢复, 消除硬切突兀感
## 场景切换在点击瞬间即可调用, 音乐先于画面响应
var _base_bgm_db := 0.0
var _tween_bgm_swap: Tween

func play_bgm_smooth(stream: AudioStream) -> void:
	if bgm_play.stream == stream and bgm_play.playing:
		return
	if _tween_bgm_swap and _tween_bgm_swap.is_valid():
		_tween_bgm_swap.kill()
		bgm_play.volume_db = _base_bgm_db
	_base_bgm_db = bgm_play.volume_db
	_tween_bgm_swap = create_tween()
	_tween_bgm_swap.tween_property(bgm_play, "volume_db", -22.0, 0.13)\
		.set_trans(Tween.TRANS_QUAD).set_ease(Tween.EASE_OUT)
	_tween_bgm_swap.tween_callback(play_bgm.bind(stream))
